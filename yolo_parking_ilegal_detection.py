...
class IllegalParkingDetector:
    def __init__(self, model_path, logger_instance):
        ...
        # {track_id: {'entry_time': float, 'class': str, 'zone_id': int}}
        self.vehicles_in_zone = {}
        
        # Kelas kendaraan yang dipantau. Untuk yolov8s.pt, 'car' adalah kelas 2.
        ...
        self.total_violations = 0 # Akumulasi total pelanggaran
    ...
    def draw_zones(self, frame):
        ...
        for i, zone_norm in enumerate(self.illegal_zones):
            ...
            cv2.fillPoly(overlay, [zone_pixels], (0, 0, 255)) # Warna dasar zona
    ...
    def process_video(self, video_path, output_path, cctv_name, show_preview=True, violation_threshold=2):
        ...
        while cap.isOpened():
            ...
            # Set untuk menyimpan ID yang terdeteksi di frame saat ini
            current_frame_track_ids = set()

            # Pastikan ada hasil tracking
                ...
                for box, track_id, cls in zip(boxes, track_ids, classes):
                    ...
                    if in_illegal_zone:
                        ...
                        if is_violation and track_id not in self.logged_violations:
                            self.logger.log_violation(cctv_name, track_id, class_name, duration)
                            self.logger.save_violation_screenshot(frame, cctv_name, track_id)
                            self.logged_violations.add(track_id)
                            self.total_violations += 1 # Tambah total pelanggaran

                        # Gambar BBox dan label
                        cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
                        label = f"{class_name} | ID: {track_id} | {self.format_duration(duration)}"
                        if is_violation:
                            label = f"{class_name} | ID: {track_id} | LIMIT: {self.format_duration(duration)}"
                        ...
                        # Gambar BBox normal untuk kendaraan yang tidak melanggar
                        cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                        label = f"{class_name} | ID: {track_id}"
                        cv2.putText(frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

            # Hapus kendaraan dari self.vehicles_in_zone jika sudah tidak terdeteksi lagi oleh tracker
            ...
            # Gambar zona dan heatmap pada frame
            ...
            cv2.putText(frame, f"Total Pelanggar: {self.total_violations}", (10, 60),
...
if __name__ == "__main__":
    ...
    zones_file = f'config/hasil_zoning/parking_zones_{args.cctv_name}.json' # Dinamis
    try:
    ...
    detector.process_video(
        ...
        show_preview=True,
        violation_threshold=10  # Dianggap pelanggaran setelah 5 detik
    )
